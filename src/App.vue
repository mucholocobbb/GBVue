<template>
  <div id="app">
    ...
    <div class="mainbtn__block">
      <button
        class="mainbtn__style"
        v-bind:class="{ mainbtn__active: item.show }"
        @click="checkItem(item.name, item.id, item.msg)"
        v-for="item of buttonsArray"
        :key="item.id"
      >
        {{ item.name }}
      </button>
...
export default {
  ...
  created() {
    this.$router
      .push({ name: "Money Notes", params: { message: "Where you money?!" } })
      .catch(() => {});
  },
};
</script>
